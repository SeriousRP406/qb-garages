...
QBCore.Functions.CreateCallback("qb-garages:server:CheckSpawnedVehicle", function(source, cb, plate)
    cb(VehicleSpawnerVehicles[plate] ~= nil and VehicleSpawnerVehicles[plate])
end)

RegisterNetEvent("qb-garages:server:UpdateSpawnedVehicle", function(plate, value)
...
QBCore.Functions.CreateCallback('qb-garages:server:jobspawnvehicle', function (source, cb, vehInfo, coords, warp)
    local veh = QBCore.Functions.SpawnVehicle(source, vehInfo.vehicle, coords, warp)
    print(veh)
    if not veh or not NetworkGetNetworkIdFromEntity(veh) then
        print('ISSUE HERE', veh, NetworkGetNetworkIdFromEntity(veh))
...
QBCore.Functions.CreateCallback("qb-garages:server:GetGarageVehicles", function(source, cb, garage, garageType, category)
    ...
    elseif garageType == "depot" then    --Depot give player cars that are not in garage only
         MySQL.query('SELECT * FROM player_vehicles WHERE (citizenid = ? AND state = ?) OR (citizenid = ? AND state = ? AND releasetime <= NOW())', {pData.PlayerData.citizenid, 3, pData.PlayerData.citizenid, 2}, function(result)
            local tosend = {}
            if result[1] then
    ...
    elseif garageType == "impound" then    --Give all cars from impound garage in impound
...
if UseEnc0dedPersistenVehicles then
    QBCore.Functions.CreateCallback("qb-garages:server:checkIsSpawned", function (plate)
...
QBCore.Functions.CreateCallback("qb-garages:server:checkOwnership", function(source, cb, plate, garageType, garage, gang, hasHouseKey)
    local src = source
    local pData = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('qb-garages:server:updateVehicle', function(state, fuel, engine, body, properties, plate, garage, location, damage)
...
RegisterNetEvent('qb-garages:server:updateVehicleState', function(state, plate, garage)
    MySQL.update('UPDATE player_vehicles SET state = ?, garage = ?, depotprice = ?, releasetime = NULL WHERE plate = ?',{state, garage, depotprice, plate})
end)
...
AddEventHandler('onResourceStart', function(resource)
    ...
    if resource == GetCurrentResourceName() then
        ...
        if AutoRespawn then
            ...
            MySQL.update('UPDATE player_vehicles SET state = 3, garage = ? WHERE state = 0', {depot})
            Wait(100)
            ImpoundRelease()
        end
    end
...
function ImpoundRelease()
    local depot = "Depot"
    MySQL.update('UPDATE player_vehicles SET state = 3, garage = ?, releasetime = NULL WHERE state = 2 AND releasetime <= NOW()', {depot})
    Wait(ImpoundReleaseInterval)
    ImpoundRelease()
end
...
RegisterNetEvent('qb-garages:server:Impound', function(plate, fullImpound, impoundtime, price, body, engine, fuel)
    local src = source
    now = os.time()
    impoundmultiplier = impoundtime * 60 * 60
    releasetime = os.date("%Y/%m/%d %X", now + impoundmultiplier)
    depot = "Depot"
    pdimpound = "pdimpound"
    ...
    if IsVehicleOwned(plate) then
        if not fullImpound then
            MySQL.query(
                'UPDATE player_vehicles SET state = ?, garage = ?, depotprice = ?, body = ?, engine = ?, fuel = ? WHERE plate = ?',
                {3, depot, price, body, engine, fuel, plate})
            TriggerClientEvent('QBCore:Notify', src, "Vehicle sent to DEPOT", {price = price})
        else
            MySQL.query(
                'UPDATE player_vehicles SET state = ?, garage = ?, body = ?, engine = ?, fuel = ?, releasetime = ? WHERE plate = ?',
                {2, pdimpound, body, engine, fuel, releasetime, plate})
            TriggerClientEvent('QBCore:Notify', src, "Vehicle sent to IMPOUND")
        end
    end
end)
...
--External Calls
--Pilot License Check
QBCore.Functions.CreateCallback('qb-garages:getLicenseStatus', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local licenseTable = Player.PlayerData.metadata["licences"]
    local licenseItem = Player.Functions.GetItemByName("pilot")
    cb(licenseTable.pilot, licenseItem)
end)
...
QBCore.Commands.Add("depot", "Send vehicle to depot with price.", {{name = "price", help = "/depot price"}}, false, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        TriggerClientEvent("qb-garages:client:ImpoundVehicle", src, false, tonumber(args[1]))
    else
        TriggerClientEvent('QBCore:Notify', src, "On Duty PD Only", 'error')
    end
end)

QBCore.Commands.Add("impound", "Send vehicle to police impound.", {}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "police" and Player.PlayerData.job.onduty then
        TriggerClientEvent("qb-garages:client:ImpoundVehicle", src, true)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("On Duty PD Only"), 'error')
    end
end)
